...
async function createFile(data: Blob) {
  ...
  return sharp(buff).toFormat("webp").toBuffer();
}
        ...
        async ({ params, body: files }) => {
          ...
          await Promise.all(
            objectEntires.map(async ([key, file]) => {
              ...
              const [image_id] = await db.insertInto("images").values({ title: key, project_id }).returning("id").execute();
          ...
          return { data: filesToSend, message: MessageEnum.success, ok: true };
        },
        {
          body: t.Files({ type: ["image"] }),
          response: ResponseWithDataSchema,
        },
      )
      .post(
