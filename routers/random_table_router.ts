...
import { checkEntityLevelPermission, getHasEntityPermission, getRandomTableOptionRelatedData } from "../database/queries";
              ...
              .$if(!!body?.relations?.random_table_options, (qb) =>
                qb.select((eb) => {
                  let rto_query = eb
                    ...
                    .orderBy((ob) => sql`NULLIF(regexp_replace(${ob.ref("random_table_options.title")}, '\\D.*', ''), '')::int`)
