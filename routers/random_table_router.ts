                      ...
                      .select([
                        ...
                        (sb) =>
                          sb
                            ...
                            .then(
                              sql`JSONB_BUILD_OBJECT('id', maps.id, 'title', maps.title, 'image_id', maps.image_id, 'type', 'maps')`,
                            )
                            .when("random_table_options.map_pin_id", "is not", null)
                            .then(
                              sql`JSONB_BUILD_OBJECT('id', map_pins.id, 'title', map_pins.title, 'icon', map_pins.icon, 'type', 'map_pins')`,
                            )
                            .when("random_table_options.graph_id", "is not", null)
                            .then(
                              sql`JSONB_BUILD_OBJECT('id', graphs.id, 'title', graphs.title, 'icon', graphs.icon, 'type', 'graphs')`,
                            )
                            ...
                            .then(
                              sql`JSONB_BUILD_OBJECT('id', images.id, 'title', images.title, 'image_id', images.id, 'type', 'images')`,
                            )
                            .end()
                            .as("related_data"),
                      ])
                      // Order by numbers if they start with numbers
                      .orderBy(
