...
export function gateway_router(app: Elysia) {
  return app.group("/gateway", (server) =>
    server.get("/access/:type/:access_id", async ({ params }) => {
      ...
      if (gateway_access) {
        try {
          const data = JSON.parse(gateway_access) as { access_id: string; entity_id: string; accessed: boolean };
          if (data.access_id === params.access_id && !data.accessed) {
            if (params.type === "characters") {
              ...
              return { data: rest, ok: true, message: MessageEnum.success };
            }
          }
        } catch (error) {
          console.error(error);
