...
export async function createPDF(body: string): Promise<Blob> {
  const html = createHTML(body);

  const logoBase64 = fs.readFileSync("./public/Logo.webp", "base64");
  const logoDataUrl = `data:image/png;base64,${logoBase64}`;

  const res = await fetch(`${process.env.BROWSER_DOMAIN}/chrome/pdf?token=${process.env.BROWSER_WS_ENDPOINT_TOKEN}`, {
