...
export async function afterCreateHandler(
  args: {
    data: {
  ...
  const { is_folder, project_id, title, label, first_name, last_name } = args.data;
  if (project_id) {
    ...
    sendNotification(project_id, {
      ...
      message: `${user?.nickname || "User"} created a new ${getSingularEntityType(entity)} ${
        is_folder ? "folder" : ""
      } - "${entityName}"`,
      entity,
      userId,
